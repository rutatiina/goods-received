...
namespace Rutatiina\GoodsReceived\Traits;

use Illuminate\Support\Facades\Auth;
use Rutatiina\Item\Models\Item;
use Rutatiina\Purchase\Models\Purchase;
...
trait Inventory
{
    ...
    private function inventoryPurchases($item)
    {
        ...
        */
        $query = Purchase::where('financial_account_code', $this->txn['credit_financial_account_code'])
    ...
    private function inventoryPurchase()
    {
        ...
        foreach($items as $item) {
            ...
            $Purchase->tenant_id = Auth::user()->tenant->id;
            $Purchase->txn_id = $this->txn['id'];
            $Purchase->date = $this->txn['date'];
            $Purchase->financial_account_code = $this->txn['debit_financial_account_code'];
            $Purchase->item_id = $item['type_id'];
            $Purchase->batch = $item['batch'];
            $Purchase->expiry = $item['expiry'];
            $Purchase->value_per_unit = ($item['rate'] / $item['units']);
            $Purchase->currency = $this->txn['base_currency'];
            $Purchase->units = $units;
            $Purchase->balance = $units;
            $Purchase->save();
        ...
        foreach($items as $index => $item) {
            ...
            if ($purchases == false || empty($purchases) ) {
                ...
                foreach($purchases as $purchase) {
                    ...
                    $inventory_purchase = Purchase::find($purchase['id']);
